    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div class="flex items-center justify-between p-5 border-b border-gray-50">
                ...
                <a href="{{ route('author.manuscripts.create') }}" class="bg-[#e8a020] text-white text-xs px-3 py-1.5 rounded-lg hover:bg-[#d4911c] transition-colors">+ Submit New</a>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($manuscripts->take(6) as $manuscript)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-5 py-3">
                                    <p class="text-xs font-medium text-gray-800 line-clamp-1 max-w-[200px]">{{ $manuscript->title }}</p>
                                </td>
                                <td class="px-5 py-3">
                                ...
                                <td class="px-5 py-3">
                                    @if(in_array($manuscript->status, ['draft', 'rejected']))
                                        <a href="{{ route('author.manuscripts.edit', $manuscript->id) }}" class="text-[#e8a020] hover:underline text-xs font-medium">Edit</a>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-12 text-center text-gray-400 text-sm">
                                    No manuscripts yet. <a href="{{ route('author.manuscripts.create') }}" class="text-[#e8a020] hover:underline">Submit your first one →</a>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="divide-y divide-gray-50">
                @forelse($messages as $message)
                    <div class="p-4 {{ is_null($message->read_at) ? 'bg-blue-50' : '' }}">
                        ...
                        <p class="text-[10px] text-gray-400 mt-2">From: {{ $message->sender?->name }} · {{ $message->created_at->diffForHumans() }}</p>
                    </div>
                @empty
