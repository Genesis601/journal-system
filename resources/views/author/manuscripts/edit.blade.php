...
@section('page-title', 'Edit Manuscript')
...
<div class="max-w-3xl">
    ...
    <div class="mb-6">
        <h2 class="text-lg font-semibold text-gray-800">Edit Manuscript</h2>
        <p class="text-sm text-gray-400 mt-0.5">Update your manuscript details and resubmit</p>
    </div>

    {{-- Rejection notice --}}
    @if($manuscript->status === 'rejected')
        <div class="bg-red-50 border border-red-100 rounded-xl p-4 mb-5">
            <p class="text-sm font-medium text-red-700 mb-1">This manuscript was rejected</p>
            @php $review = $manuscript->reviews()->latest()->first(); @endphp
            @if($review)
                <p class="text-xs text-red-600 leading-relaxed">{{ $review->comments }}</p>
            @endif
        </div>
    @endif

    <form method="POST"
          action="{{ route('author.manuscripts.update', $manuscript->id) }}"
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-6 flex flex-col gap-5">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $manuscript->title) }}"
                ...
                <select name="journal_id"
                    ...
                    @foreach($journals as $journal)
                        <option value="{{ $journal->id }}"
                            {{ old('journal_id', $manuscript->journal_id) == $journal->id ? 'selected' : '' }}>
                            {{ $journal->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="abstract" rows="6"
                          class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-700 outline-none focus:border-[#e8a020] transition-colors resize-none @error('abstract') border-red-300 @enderror">{{ old('abstract', $manuscript->abstract) }}</textarea>
                @error('abstract')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="keywords"
                       ...
                       class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-700 outline-none focus:border-[#e8a020] transition-colors @error('keywords') border-red-300 @enderror">
                @error('keywords')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-700 mb-1.5">
                    Replace File <span class="text-gray-400 font-normal">(optional)</span>
                </label>
                <div class="border-2 border-dashed border-gray-200 rounded-lg p-6 text-center hover:border-[#e8a020] transition-colors cursor-pointer"
                    ...
                    <p class="text-sm text-gray-500">Click to upload a new file</p>
                    <p class="text-xs text-gray-400 mt-1">Current file will be kept if no new file is selected</p>
