 @extends('layouts.dashboard')

@section('title', 'Editor Dashboard')
@section('page-title', 'Editor Dashboard')

@section('sidebar-nav')
@php
    $navItems = [
        ['route' => 'editor.dashboard', 'label' => 'Dashboard', 'icon' => 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6'],
        ['route' => 'editor.manuscripts.index', 'label' => 'Manuscripts', 'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
        ['route' => 'editor.messages.index', 'label' => 'Messages', 'icon' => 'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z'],
    ];
@endphp
<div class="flex flex-col gap-1">
    @foreach($navItems as $item)
        <a href="{{ route($item['route']) }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs transition-colors
                  {{ request()->routeIs($item['route']) ? 'bg-[#e8a020] text-white' : 'text-[#a0b4cc] hover:text-white hover:bg-[#1e3a5a]' }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $item['icon'] }}"/>
            </svg>
            {{ $item['label'] }}
        </a>
    @endforeach
</div>
@endsection

@section('content')

{{-- STATS --}}
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-xl border border-gray-100 p-5">
        <p class="text-xs text-gray-400 mb-1">New Submissions</p>
        <p class="text-2xl font-semibold text-blue-500">{{ $stats['submitted'] }}</p>
    </div>
    <div class="bg-white rounded-xl border border-gray-100 p-5">
        <p class="text-xs text-gray-400 mb-1">Under Review</p>
        <p class="text-2xl font-semibold text-amber-500">{{ $stats['under_review'] }}</p>
    </div>
    <div class="bg-white rounded-xl border border-gray-100 p-5">
        <p class="text-xs text-gray-400 mb-1">Published</p>
        <p class="text-2xl font-semibold text-green-500">{{ $stats['published'] }}</p>
    </div>
    <div class="bg-white rounded-xl border border-gray-100 p-5">
        <p class="text-xs text-gray-400 mb-1">Rejected</p>
        <p class="text-2xl font-semibold text-red-500">{{ $stats['rejected'] }}</p>
    </div>
</div>

<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">

    {{-- RECENT SUBMISSIONS --}}
    <div class="lg:col-span-2 bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div class="flex items-center justify-between p-5 border-b border-gray-50">
            <h2 class="text-sm font-semibold text-gray-800">Recent Submissions</h2>
            <a href="{{ route('editor.manuscripts.index') }}"
               class="text-[#e8a020] text-xs hover:underline">View all →</a>
        </div>
        <div class="divide-y divide-gray-50">
            @forelse($recentSubmissions as $manuscript)
                <div class="p-4 hover:bg-gray-50 transition-colors">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex-1 min-w-0">
                            <p class="text-xs font-medium text-gray-800 line-clamp-1 mb-1">
                                {{ $manuscript->title }}
                            </p>
                            <div class="flex items-center gap-2 flex-wrap">
                                <span class="text-[10px] text-gray-400">
                                    By {{ $manuscript->author?->name }}
                                </span>
                                <span class="text-gray-200">·</span>
                                <span class="text-[10px] text-gray-400">
                                    {{ $manuscript->journal?->title }}
                                </span>
                                <span class="text-gray-200">·</span>
                                <span class="text-[10px] text-gray-400">
                                    {{ $manuscript->created_at->diffForHumans() }}
                                </span>
                            </div>
                        </div>
                        <div class="flex items-center gap-2 flex-shrink-0">
                            @php
                                $statusColors = [
                                    'submitted'    => 'bg-blue-50 text-blue-700',
                                    'under_review' => 'bg-amber-50 text-amber-700',
                                ];
                            @endphp
                            <span class="text-[10px] font-medium px-2 py-1 rounded {{ $statusColors[$manuscript->status] ?? 'bg-gray-100 text-gray-500' }}">
                                {{ ucfirst(str_replace('_', ' ', $manuscript->status)) }}
                            </span>
                            <a href="{{ route('editor.manuscripts.show', $manuscript->id) }}"
                               class="bg-[#0f2744] text-white text-[10px] px-3 py-1.5 rounded-lg hover:bg-[#1a3d5c] transition-colors">
                                Review
                            </a>
                        </div>
                    </div>
                </div>
            @empty
                <div class="p-12 text-center text-gray-400">
                    <p class="text-3xl mb-2">📭</p>
                    <p class="text-sm">No pending submissions</p>
                </div>
            @endforelse
        </div>
    </div>

    {{-- MESSAGES --}}
    <div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div class="flex items-center justify-between p-5 border-b border-gray-50">
            <h2 class="text-sm font-semibold text-gray-800">Recent Messages</h2>
            <a href="{{ route('editor.messages.index') }}"
               class="text-[#e8a020] text-xs hover:underline">View all →</a>
        </div>
        <div class="divide-y divide-gray-50">
            @forelse($messages as $message)
                <div class="p-4 {{ is_null($message->read_at) ? 'bg-blue-50' : '' }}">
                    <div class="flex items-start justify-between gap-2 mb-1">
                        <p class="text-xs font-medium text-gray-800 line-clamp-1">
                            {{ $message->subject }}
                        </p>
                        @if(is_null($message->read_at))
                            <span class="w-2 h-2 bg-blue-500 rounded-full flex-shrink-0 mt-1"></span>
                        @endif
                    </div>
                    <p class="text-xs text-gray-500 line-clamp-2">{{ $message->body }}</p>
                    <p class="text-[10px] text-gray-400 mt-1">
                        {{ $message->sender?->name }} · {{ $message->created_at->diffForHumans() }}
                    </p>
                </div>
            @empty
                <div class="p-8 text-center text-gray-400">
                    <p class="text-3xl mb-2">✉️</p>
                    <p class="text-xs">No messages yet</p>
                </div>
            @endforelse
        </div>
    </div>

</div>

@endsection